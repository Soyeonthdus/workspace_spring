...
public class SonataTest {
	public static void main(String args[]) {
			Sonata myCar = new Sonata(30, "2023년형 소나타", 4); //스프링을 경유하지 않는 방식
			//myCar.carName = "2023년형 소나타";
			System.out.println(myCar.toString());
			
			ApplicationContext context = 
					//스프링을 경유하기 위해 필요한 것 -> 스프링을 경유하기 위한 고급기술이다
					//최근에는 이렇게도 안쓰고 @어노테이션 하나로 처리해버림
					new ClassPathXmlApplicationContext("com\\example\\demo\\di\\car.xml");
			Sonata himCar = (Sonata)context.getBean("himCar"); //생성자 주입 없음
 ...
 * : 역제어란 객체의 생성 및 생명주기에 대한 제어권이 개발자에게서 Container에게 넘어갔다는 것을 의미함
 * 기존 방식
 ...
 * 
 * IoC사용시
 * 제어권이 Controller에게 넘어가 객체의 생명주기를  Conatainer가 전담하게 됨 - 예) 서블릿, EJB
 * 장점 : 컴포넌트 간의 결합도가 낮아져 컴포넌트의 재사용성 및 확작성 증가하고 컴포넌트의 의존관계 변경이 쉬움
 * 
 * Spring Container는 BeanFactory와 ApplicationContext 라는 두가지 유형의 컨테이너를 제공함
 * 
 * BeanFactory
 * : 객체를 관리하는 고급 설정 기법 제공
 * : spring-bean.jar에 속함
 * : Bean 들에 대한 생성, 소멸 , Life Cycle과 같은 컨테이너가 빈을 관리하기 위해 필요한 기본적 기능 제공
 * : 게으른....
 * : lazy loading
 ...
 * : 웹 어플리케이션에서 사용하기 위한 WebApplicationContext와 같은 특정 ApplicationContext와의 통함기능을 추가 제공함
 * : 이른 인스턴스화
 * : Context를 시작할 때 모든 Singleton Bean을 미리 로딩함으로써 그 bean이 필요할 때 즉시 사용할 수 있게 보장해줌
